...
def filtrar_banda(data, sfreq, low_freq=0.5, high_freq=2.0):
    return mne.filter.filter_data(data, sfreq, l_freq=low_freq, h_freq=high_freq)

# Cargar archivo BDF
archivo_bdf = "./Testdata4.bdf"
...
try:
    ...
    canal_0_filtrado = filtrar_banda(canal_0, sfreq)
    ...
    intervalos = 6.0  # Duración de los intervalos en segundos
    ...
    tiempos_hr = []  # Lista para almacenar los tiempos correspondientes a las frecuencias cardíacas
    ...
    for inicio in np.arange(0, total_time - intervalos + 1, desplazamientofloat):  
        ...
        tiempos_hr.append(inicio)  # Almacenar el tiempo de inicio del intervalo
    ...
    if hr_list:
        ...
        # Graficar la frecuencia cardíaca en función del tiempo
        plt.figure(figsize=(12, 6))
        for i in range(len(hr_list)):
            if hr_list[i] > 0:  # Solo graficar si la frecuencia es mayor que 0
                plt.hlines(hr_list[i], tiempos_hr[i], tiempos_hr[i] + intervalos, colors='blue', alpha=0.5)
        
        plt.axhline(promedio_hr, color='orange', linestyle='--', label='Promedio de Frecuencia Cardíaca')
        plt.title("Frecuencia Cardíaca en Función del Tiempo")
        plt.xlabel("Tiempo (segundos)")
        plt.ylabel("Frecuencia Cardíaca (bpm)")
        plt.xlim(0, total_time)  # Limitar el eje x al tiempo total
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
